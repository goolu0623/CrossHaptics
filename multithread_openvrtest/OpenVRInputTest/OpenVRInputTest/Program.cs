...
namespace OpenVRInputTest {
    class Program {
        ...
        public static Queue<Tuple<string, string>> output_data = new Queue<Tuple<string, string>>();
        ...
        private static void Worker() {
            ...
            while (true) {
                ...
                if (sw.ElapsedMilliseconds > (int)(1000 / DataFrameRate)) {
                ...
                try {
                    //while (sw.ElapsedMilliseconds <= (int)(1000 / DataFrameRate)) { // framerate還不到的話繼續撈資料
        ...
        private static void Writer() {
            ...
            while (true) {
                ...
                if (output_data.Count != 0) {
                    ...
                    LogWriterTest.LogWriter.LogWrite(temp.Item1, "console.txt");
                    // 37~42包含output
                    if (temp.Item2 != "Pose") 
                        Utils.PrintError(text: $"{temp.Item1}");
                    else 
                        Utils.Print(text: $"{temp.Item1}");
                }
            }
        }
    }
